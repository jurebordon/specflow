...
- Community agents (install from [VoltAgent](https://github.com/VoltAgent/awesome-claude-code-subagents) if needed)
...
- Refine workflows based on what you actually do
...
For DBT projects, document key patterns in CLAUDE.md or OVERVIEW.md:

- **Model layers**: staging (1:1 with source), intermediate (business logic), marts (consumption)
- **Testing**: unique + not_null on PKs, relationship tests for FKs
- **Incremental models**: for fact tables >1M rows
- **Naming**: `stg_[source]__[table]`, `int_[entity]__[transform]`, `dim_/fct_` prefixes

For specialist implementation support, install community agents from [VoltAgent](https://github.com/VoltAgent/awesome-claude-code-subagents).
